...
- **Data storage (TSM & TSI):** data points are stored in time-structured merge
  ...
  that are sorted and compressed. In the event of
  [high cardinality](/glossary/high-cardinality/) (i.e., large number of elements
  in a set), performance can decrease. The proprietary InfluxDB Cloud (IOx)
  claims high cardinality is not an issue anymore.
...
- **Continuous Aggregation:** TimescaleDB also supports
  ...
  fast. Behind the scenes, the data is stored using PostgreSQL
  [materialized views](/glossary/materialized-view/); additional storage is
  needed as a trade-off for faster queries. For time-based aggregations (e.g.,
  average temperature between 3 pm and 4 pm vs. the exact temperature at 3 pm),
  scanning fewer data for each aggregation can help performance-wise.
