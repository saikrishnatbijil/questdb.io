...
“Upload” icon on the left-hand panel, and import the
...
Navigate to `localhost:4000/#/schema` and click on the Ethereum table we
...
will create [materialized](/glossary/materialized-view/) rollups of specified
dimensions and measures internally, and use aggregate awareness logic to route
the queries. This logic will use the most optimal pre-aggregation available to
serve the query instead of processing the raw dataset. You can define
pre-aggregations within your data model as shown below.
...
3. SQL API: If you're querying data using standard ANSI SQL format, you can
   ...
   need to learn additional syntax—just use SQL to interact with your data. This
   is especially useful if you are working with BI tools, dashboards, or data
   science models.
