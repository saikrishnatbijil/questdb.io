...
keywords:
  ...
  - monitoring
  - uptime
tags: [tutorial, python, fastapi, monitoring]
image: /img/blog/shared/og-python.webp
...
<Banner
  ...
  height={433}
  src="/img/blog/2021-01-13/banner.jpg"
  width={650}
>
...
<Screenshot
  ...
  height={284}
  src="/img/blog/2021-01-13/end-result.webp"
...
`psycopg` client library, we need to install `libpq-dev` package on our
...
`http://127.0.0.1:8000/redoc`, you can see the
generated documentation for the endpoint, or you can check it working at
`http://127.0.0.1:8000/signals`, though it won't
return any data yet.
...
[Postgres compatibility](/docs/reference/api/postgres), you can use
...
`http://127.0.0.1:9000` and create a new table by running
the following query:
...
<Screenshot
  ...
  height={284}
  src="/img/blog/2021-01-13/interactive-console.webp"
  title="Finished statuspage indicator"
  width={650}
/>
...
interface of QuestDB and Postgres using connection pooling to connect to it.

To be able to reuse the pool later on, create a new file in the `app` package
...
To set up a schema that represents the table in the database, create a `models.py`
in the `app` package, containing the schema definition:
...
database. First, import the `pool` and `Signal` schema and then extend the
...
`app/tasks.py` referenced in celery as the dotted path before.
...
For the sake of simplicity, we are going to allow all origins. We highly
...
<Screenshot
  ...
  height={284}
  src="/img/blog/2021-01-13/end-result-2.webp"
  title="Finished statuspage indicator"
  width={650}
/>
