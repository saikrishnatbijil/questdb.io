...
use. On most operating systems, it is straightforward to increase this limit,
...
I increased the soft limit and changed the hard limit to `Integer.MAX` (basically
unlimited).
...
switch to the soft limit, so the existing hard limit should also be set as
soft limit. Otherwise, suddenly we could start using a much lower value, as
defined by the soft limit. The default could be as low as 256!
